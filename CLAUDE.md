...
- **Deploy:** runs in Docker (required) on the external `pg_net` network, DB at `pg_db:5432`. Own image `ops-dashboard:${USER_ID}` built from the repo Dockerfile; identity is decided by `entrypoint.sh` (`RUN_USER` defaults to **svc**; dev runs pass `RUN_USER=<you>`) — no `user:` pin. `node_modules` lives in-tree, installed by `build.sh` (no shared cache mount). **Unlike** the batch apps (one-shot `docker compose run` cron jobs), this is a **long-running service**: `docker compose up -d`, published port `${HOST_PORT}` (dev `8081`, release `8080`), `command: node index.js serve`. The dev clone and the release copy are separated by `COMPOSE_PROJECT_NAME` (`ops-dashboard-dev` vs `ops-dashboard`) — without that split, a dev `up -d` would recreate the production container. See `markdown/ENVIRONMENT.md`.
