...
| 32 | `prompt_32_entity_workspace_and_incident_drilldown.txt` | Completed (2026-07-24; pending independent review) | `#entity=` is a dedicated read-only workspace correlating the entity's paged incident records (`/api/incidents?entity=` + additive scopedTotal/firstSeen), current connectivity truth, and 24h run-log signals via `GET /api/entities/:id`; `#system=` is a permanent alias; every equipment link is canonical. 171 tests + 32-check live gate + browser gate pass. See PHASE_LOG. |
