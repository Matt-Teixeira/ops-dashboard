...
| `SYSTEMS_WINDOW_HOURS` | Per-system correlation view window (hours) | default `24`; clamped `1..168`; `GET /api/systems` + `GET /api/systems/:id` — cross-app per-`sme` warn/error rollup over `util.app_run_logs.warn_error_logs` (no `verbose_log`), partition-pruned. No extra grant (reads `util` + `alert`) |
