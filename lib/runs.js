...
/** A `dt` value that parses as a real timestamp, else null. */
function validDt(value) {
  return typeof value === "string" && !Number.isNaN(Date.parse(value)) ? value : null;
}

/**
 * Run start/end/duration from the first/last event `dt` in verbose_log.
 * Defensive: invalid/missing `dt` -> null, and a negative span (events out of
 * order) -> null rather than a misleading negative duration. Mirrors the SQL in
 * db/queries.js so the grid and drill-down agree on failure behavior.
 */
function timing(verboseLog) {
  ...
  const startedAt = validDt(events[0] && events[0].dt);
  const endedAt = validDt(events[events.length - 1] && events[events.length - 1].dt);
  let durationMs = null;
  if (startedAt && endedAt) {
    const span = new Date(endedAt).getTime() - new Date(startedAt).getTime();
