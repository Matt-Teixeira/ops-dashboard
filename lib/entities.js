...
/** Merge grouped category/source rows into finalized, ordered entity summaries. */
function mergeSummaryRows(rows) {
  ...
  return Array.from(grouped.values(), finalize).sort(entityOrder);
}

function shapeEntityResponse(rows, asOf = new Date().toISOString()) {
  const all = mergeSummaryRows(rows);
...
/**
 * One entity's workspace context (Phase 32): correlate — never conflate — the
 * three read-only sources for a single safe id. `incidentSummary` is the same
 * finalized shape as one Phase 30 card (or null when the entity has no incident
 * rows); `connectivity` and `signals` arrive pre-shaped by lib/connectivity and
 * lib/systems. Pure; owns no fetch/window policy beyond echoing windowHours.
 */
function shapeEntityContext({ entity, summaryRows, connectivity, signals, windowHours, asOf } = {}) {
  const summaries = mergeSummaryRows(summaryRows);
  // A bound single-entity query returns rows for exactly one entity; a caller
  // passing the unscoped read still resolves the requested id defensively.
  const incidentSummary = summaries.find((item) => item.entity === entity) || null;
  return {
    asOf: instant(asOf) || new Date().toISOString(),
    entity,
    entityKind: classifyEntity(entity),
    incidentSummary,
    connectivity: Array.isArray(connectivity) ? connectivity : [],
    signalWindowHours: count(windowHours),
    signals: Array.isArray(signals) ? signals : [],
  };
}

/** True when every workspace source is empty — the endpoint's 404 rule. */
function entityContextIsEmpty(context) {
  return !context ||
    (context.incidentSummary == null &&
      (!Array.isArray(context.connectivity) || context.connectivity.length === 0) &&
      (!Array.isArray(context.signals) || context.signals.length === 0));
}
